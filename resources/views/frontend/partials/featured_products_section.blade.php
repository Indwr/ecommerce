                    ...
                    <div class="product">
                        <div class="product-img  mb-15">
                            ...
                            <div class="product-action-2">
                                            <button title="Wishlist" onclick="addToWishList({{ $product->id }})"
                                            ...
                                            <button title="bag"
                                                    onclick="showAddToCartModal({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to cart') }}" data-placement="left"
                                                    >
                                               <i class="las la-shopping-cart"></i>
                                </button>
                                
                                <button title="zoom"><i class="las la-search-plus"></i></button>
                                
                                <button title="Compare"
                                        onclick="addToCompare({{ $product->id }})" data-toggle="tooltip" data-title="{{ translate('Add to compare') }}" data-placement="left"
                                        ><i class="las la-sync-alt"></i></button>
                                        </div>
                            ...
                            <h5 class="pro-name">
                             <a href="{{ route('product', $product->slug) }}" class="d-block text-reset">{{  $product->getTranslation('name')  }}</a>
                               ...
                               @if(home_base_price($product->id) != home_discounted_base_price($product->id))
                                    <del class="">{{ home_base_price($product->id) }}</del>
                                @endif
                                <span class="">{{ home_discounted_base_price($product->id) }}</span>
                            </h4>
